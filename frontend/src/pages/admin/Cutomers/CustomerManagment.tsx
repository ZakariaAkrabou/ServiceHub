...
import { mockCustomers, avatarColors, type CustomerRole, type Customer } from "../Providers/data/customersMockData";
...
function RoleBadge({ role }: { role: CustomerRole }) {
    return (
        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold w-fit ${role === "service_provider"
...
function ActionButtons({ onOpenDetails, onDeleteClick, onBanClick }: { onOpenDetails: () => void, onDeleteClick: () => void, onBanClick: () => void }) {
    return (
        <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
            <button title="View Details" onClick={onOpenDetails} className="cursor-pointer text-gray-400 hover:text-indigo-600 transition p-1.5 rounded-lg hover:bg-indigo-50">
                <EyeIcon />
            </button>
            <button title="Ban Customer" onClick={onBanClick} className="cursor-pointer text-red-500 hover:text-red-600 transition p-1.5 rounded-lg hover:bg-red-50">
                <BanIcon />
            </button>
            <button title="Delete" onClick={onDeleteClick} className="cursor-pointer text-gray-400 hover:text-red-500 transition p-1.5 rounded-lg hover:bg-red-50">
                <DeleteIcon />
            </button>
        </div>
    );
    ...
    return (
        <div style={{ fontFamily: "'DM Sans', sans-serif" }} className="min-h-screen bg-gray-50 p-4 sm:p-6">
            ...
            <div className="mb-5">
                ...
                <p className="text-sm text-gray-500 mt-0.5">View and manage registered customer accounts</p>
            </div>
                ...
                <>
                    ...
                    <div className="hidden md:block bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        <div className="overflow-x-auto">
                            <div className="grid grid-cols-[minmax(130px,1fr)_minmax(110px,1fr)_minmax(160px,2fr)_minmax(130px,1.2fr)_minmax(130px,1fr)_auto] gap-4 px-6 py-3 border-b border-gray-100 text-xs font-semibold text-gray-400 uppercase tracking-wide min-w-[680px]">
                            ...
                            {filtered.map((customer, i) => (
                                <div
                                    key={customer.id}
                                    className={`grid grid-cols-[minmax(130px,1fr)_minmax(110px,1fr)_minmax(160px,2fr)_minmax(130px,1.2fr)_minmax(130px,1fr)_auto] gap-4 px-6 py-4 items-center border-b border-gray-50 hover:bg-gray-50/60 transition-colors min-w-[680px] ${i === filtered.length - 1 ? "border-b-0" : ""
                                        }`}
                                >
                                    <div className="flex items-center gap-2.5 min-w-0">
                                        ...
                                        <span className="text-sm font-semibold text-gray-800 truncate">{customer.firstName}</span>
                                    </div>

                                    <span className="text-sm text-gray-700 truncate">{customer.lastName}</span>
                                    ...
                                    <ActionButtons onOpenDetails={() => setSelectedCustomer(customer)} onDeleteClick={() => setCustomerToDelete(customer)} onBanClick={() => setCustomerToBan(customer)} />
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            {customerToDelete && (
                <CustomerDeleteConfirmationModal
                    ...
                    onConfirm={() => {
                        console.log("Delete customer confirmed:", customerToDelete.id);
                        // Proceed to dispatch deletion via API here
                    }}
                />
            )}
            ...
            {customerToBan && (
                <CustomerBanConfirmationModal
                    ...
                    onConfirm={(reason, duration) => {
                        console.log("Ban customer confirmed:", customerToBan.id, "| Reason:", reason, "| Duration:", duration);
                        // Proceed to dispatch ban via API here
                    }}
                />
            )}
        </div>
