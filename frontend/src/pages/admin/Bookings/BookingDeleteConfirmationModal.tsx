...
interface BookingDeleteConfirmationModalProps {
  booking: Booking;
  onClose: () => void;
  onConfirm: () => void;
}

export default function BookingDeleteConfirmationModal({ booking, onClose, onConfirm }: BookingDeleteConfirmationModalProps) {
  return (
      ...
      <div className="relative w-full max-w-md bg-white rounded-3xl shadow-xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-start justify-between px-6 pt-6 pb-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-gray-900 leading-tight">Delete Booking?</h3>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          <p className="text-sm text-gray-600">
            Are you sure you want to delete Booking <strong className="text-gray-900">#{booking.id}</strong>? This action cannot be undone and the record will be permanently removed.
          </p>
        </div>
