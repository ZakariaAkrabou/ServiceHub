...
export default function ServicesManagement() {
  ...
  const filteredServices = servicesList.filter(service => {
    const matchesSearch = service.name.toLowerCase().includes(searchTerm.toLowerCase()) || service.providerName.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="flex flex-col gap-6 md:gap-8 pb-10 min-h-screen bg-[#F8FAFC] px-4 sm:px-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-2xl md:text-3xl font-black text-[#081D3A] tracking-tight">Services Catalog</h1>
          <p className="text-xs md:text-sm font-medium text-slate-500">Manage and oversee platform service offerings</p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-6 md:gap-8">
        ...
        {totalPages > 0 && (
          <div className="px-6 py-6 md:px-8 md:py-8 mt-4 bg-white rounded-4xl md:rounded-[3rem] shadow-[0_20px_50px_rgba(8,29,58,0.03)] border border-slate-100 flex flex-col lg:flex-row items-center justify-between gap-6">
            <div className="flex flex-col items-center lg:items-start gap-1">
              <span className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest">Data Visualization</span>
        ...
        {filteredServices.length === 0 && (
          <div className="bg-white rounded-[3rem] p-20 border border-dashed border-slate-200 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-slate-500 max-w-xs mx-auto text-sm">We couldn't find any services matching your criteria. Try adjusting your filters.</p>
          </div>
        )}
      </div>
