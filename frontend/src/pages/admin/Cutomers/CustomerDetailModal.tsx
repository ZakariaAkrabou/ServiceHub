...
import { type Customer, avatarColors } from "../Providers/data/customersMockData";
        ...
        <div className="h-24 bg-gray-50 border-b border-gray-100 flex items-start justify-between px-6 pt-6">
           <div className="flex items-center gap-4">
              <div 
                ...
                style={{ backgroundColor: avatarColors[customer.avatar] || "#6366f1" }}
              >
                {customer.avatar}
              </div>
              <div className="pb-2">
                <h3 className="text-xl font-bold text-[#081D3A] leading-tight">{customer.firstName} {customer.lastName}</h3>
                <div className="flex items-center gap-2 mt-1">
                  <p className="text-xs font-mono text-gray-500">{customer.id}</p>
                  <span className="text-gray-300">&bull;</span>
                  <span className={`inline-flex shrink-0 items-center justify-center text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wider ${customer.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
                    {customer.status}
                  </span>
                  <span className="text-gray-300">&bull;</span>
                  <span className={`inline-flex shrink-0 items-center justify-center text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wider ${isServiceProvider ? "bg-purple-50 text-purple-700 border border-purple-200" : "bg-blue-50 text-blue-600 border border-blue-100"}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-center gap-3 text-sm text-gray-700">
                ...
                <div>
                  ...
                  <p className="font-semibold text-gray-900">{customer.joinedDate}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5 flex items-center justify-between">
                <div>
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1 flex items-center gap-1.5"><ShoppingBag className="w-3 h-3"/> Total Bookings</p>
                  <p className="font-bold text-xl text-[#081D3A]">{customer.totalBookings}</p>
                </div>
                <div className="h-10 w-px bg-gray-200"></div>
                <div>
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1 flex items-center gap-1.5"><CreditCard className="w-3 h-3"/> Total Spent</p>
                  <p className="font-bold text-xl text-[#081D3A]">${customer.totalSpent.toFixed(2)}</p>
                </div>
              </div>

              <div className="bg-blue-50 border border-blue-100 rounded-xl p-3.5 text-[13px] text-blue-800 font-medium">
                Customer is currently marked as <strong>{customer.status}</strong> and acts as a <strong>{isServiceProvider ? 'service provider' : 'standard customer'}</strong> on the platform.
              </div>
            </div>
