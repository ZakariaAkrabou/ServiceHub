...
import { mockupServices, type ServiceItem } from "./services";
...
const ServiceDetail: React.FC = () => {
  ...
  // Find service and initialize reviews
  useEffect(() => {
    const foundService = mockupServices.find((s) => s.id === id);
    if (foundService) {
      setService(foundService);

      // Seed reviews based on the service's existing review count
      setReviewsList([
        {
          id: "r1",
          ...
          rating: 5,
          date: "May 10, 2026",
          comment: `Absolutely incredible service! The team from ${foundService.provider} was on time, extremely thorough, and very professional. Highly recommended!`,
        },
        {
          id: "r2",
          author: "Liam Johnson",
          ...
          rating: 4,
          date: "April 28, 2026",
          comment: `Very good quality work. They handled the ${foundService.subCategory || foundService.category} efficiently. Will definitely book again.`,
        },
        {
          id: "r3",
          author: "Olivia Smith",
          ...
          rating: 5,
          date: "March 15, 2026",
          comment: `Highly recommended! Extremely polite and did a fantastic job cleaning up afterwards.`,
        },
      ]);
    }
  }, [id]);
    ...
    return (
      <div className="min-h-screen bg-[#F5F0E8]/20 flex flex-col justify-between">
        ...
        <div className="max-w-7xl mx-auto px-6 py-32 text-center">
          <h2 className="text-2xl font-bold text-[#1A1A2E]">Service not found</h2>
          <button
  ...
  // Seed mock clients who have dealt with this provider
  const mockClients: Client[] = [
    { name: "Emily Watson", avatar: "EW", date: "May 2026", task: service.subCategory || service.category, status: "Completed" },
    { name: "Michael Chang", avatar: "MC", date: "April 2026", task: service.subCategory || service.category, status: "Completed" },
    { name: "Jessica Taylor", avatar: "JT", date: "March 2026", task: service.category, status: "Completed" },
    { name: "David Miller", avatar: "DM", date: "February 2026", task: "General Consulting", status: "Completed" },
  ];

  const handleBookService = () => {
    setBookingSuccess(true);
    setHasBooked(true);
    // Scroll to the review section or booking message
    setTimeout(() => {
  ...
  const averageRating = reviewsList.length > 0 ? (totalStars / reviewsList.length).toFixed(1) : service.rating.toFixed(1);
  ...
  return (
    ...
    >
      ...
      {/* Hero Section */}
      <section className="relative pt-32 pb-20 bg-[#0a1628] border-b border-black/3 overflow-hidden min-h-[400px] flex flex-col justify-center text-white">
        <div className="ccw__grid" />
        <div className="absolute inset-0 bg-gradient-to-r from-[#0a1628]/95 via-[#0a1628]/80 to-transparent z-5" />
        
        {/* Full Screen Bleed Background Hero Image */}
        <img 
          src={service.image} 
          alt={service.name} 
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 w-full flex flex-col items-start anim-fade-up">
          ...
          <div className="flex flex-wrap items-center gap-6 text-sm text-white/80">
            <div className="flex items-center gap-1">
              ...
              <span>•</span>
              <span className="font-semibold">{reviewsList.length} Reviews</span>
            </div>
            <span>•</span>
            ...
            <div className="bg-white rounded-3xl p-8 border border-black/4 shadow-xs anim-fade-up">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                <div className="flex items-start gap-3 p-4 bg-[#F5F0E8]/20 rounded-2xl border border-black/3">
                  ...
                  <div>
                    <h4 className="text-xs font-bold uppercase tracking-wider text-[#1A1A2E]">Damage Liability Protection</h4>
                    <p className="text-[11px] text-black/50 mt-1">Every booking is automatically covered under our secure warranty scheme.</p>
                  </div>
                </div>
                
                <div className="flex items-start gap-3 p-4 bg-[#F5F0E8]/20 rounded-2xl border border-black/3">
                  <CheckCircle className="text-[#C9A84C] shrink-0 mt-0.5" size={20} />
                  <div>
                    <h4 className="text-xs font-bold uppercase tracking-wider text-[#1A1A2E]">Certified Professionals</h4>
                    <p className="text-[11px] text-black/50 mt-1">Our providers pass standard vetting, background checks, and identity screenings.</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl p-8 border border-black/4 shadow-xs anim-fade-up">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {mockClients.map((client, idx) => (
                  <div key={idx} className="flex items-center justify-between p-4 rounded-2xl border border-black/3 bg-[#F5F0E8]/10 hover:bg-[#F5F0E8]/30 transition duration-300">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-[#1A1A2E] text-[#C9A84C] flex items-center justify-center font-bold text-xs">
                        {client.avatar}
                      </div>
                      <div>
                        <h4 className="text-xs font-bold text-[#1A1A2E]">{client.name}</h4>
                        <span className="text-[10px] text-black/45 block mt-0.5">{client.task}</span>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <span className="block text-[9px] text-black/40 mt-1 font-semibold">{client.date}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white rounded-3xl p-8 border border-black/4 shadow-xs anim-fade-up">
              <h2 className="text-xl font-bold uppercase tracking-wider text-[#1A1A2E] mb-6 border-b border-black/5 pb-3 flex items-center justify-between">
                ...
                <span className="text-xs font-semibold text-[#C9A84C] flex items-center gap-1 normal-case tracking-normal">
                  <Star size={14} className="fill-[#C9A84C] text-[#C9A84C]" /> {averageRating} rating
                </span>
              </h2>
                ...
                {hasBooked ? (
                  <form onSubmit={handleReviewSubmit} className="space-y-4">
                    <h3 className="text-xs font-bold uppercase tracking-widest text-[#1A1A2E] flex items-center gap-2">
                      <MessageSquare size={16} className="text-[#C9A84C]" /> Write Your Review
                    </h3>
                    
                    {/* Star Interactive Selector */}
                    <div>
                      <span className="block text-[11px] font-bold text-black/50 mb-2 uppercase tracking-wide">Your Rating:</span>
                      <div className="flex items-center gap-1.5">
                        {[1, 2, 3, 4, 5].map((star) => {
                          const isActive = hoveredStar !== null ? star <= hoveredStar : star <= newRating;
                          return (
                            <button
                              key={star}
                              type="button"
                    ...
                    {/* Review text area */}
                    <div>
                      <label htmlFor="review-comment-input" className="block text-[11px] font-bold text-black/50 mb-2 uppercase tracking-wide">
                        Your Feedback:
                      </label>
                      <textarea
                  ...
                  <div className="text-center py-4 space-y-2">
                    ...
                    <h4 className="text-xs font-bold uppercase tracking-wider text-[#1A1A2E]">Review Submission Locked</h4>
                    <p className="text-[11px] text-black/50 max-w-sm mx-auto leading-normal">
                      Only verified clients who have booked this service can leave feedback. Please complete a booking using the sidebar card to unlock writing a review.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-6">
                {reviewsList.map((review) => (
                  <div key={review.id} className="border-b border-black/4 pb-6 last:border-b-0 last:pb-0">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="text-xs font-bold text-[#1A1A2E]">{review.author}</h4>
                          <span className="text-[10px] text-black/40 font-semibold">{review.date}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-0.5">
                        {[...Array(5)].map((_, i) => (
                          <Star
                            ...
                            size={12}
                            className={`${
                              i < review.rating ? "fill-[#C9A84C] text-[#C9A84C]" : "text-black/10"
                            }`}
                          />
                        ))}
                      </div>
          ...
          <div className="space-y-6">
            ...
            {bookingSuccess && (
              <div className="bg-[#1A1A2E] border-2 border-[#C9A84C] text-white p-5 rounded-2xl shadow-xl flex items-start gap-3 animate-[pageFadeUp_0.4s_ease-out]">
                <UserCheck className="text-[#C9A84C] shrink-0 mt-0.5" size={24} />
                <div>
                  <h4 className="text-xs font-bold uppercase tracking-wider text-[#C9A84C]">Booking Successful!</h4>
                  <p className="text-[10px] text-white/80 mt-1 leading-normal">
                    You have successfully booked this service. The provider has been notified. You can now leave a review in the feedback section below.
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="bg-white rounded-3xl p-6 border border-black/4 shadow-sm sticky top-28">
              <span className="text-[9px] uppercase font-bold tracking-widest text-black/35 block mb-1">Pricing Package</span>
              <div className="flex items-baseline gap-1.5 border-b border-black/5 pb-4 mb-4">
                <span className="text-3xl font-black text-[#1A1A2E] font-serif">${service.price}</span>
                <span className="text-xs text-black/40 font-semibold">/ Flat Rate</span>
              </div>
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="flex items-center justify-between text-xs text-black/60">
                  ...
                  <span className="font-bold text-[#C9A84C]">{service.provider}</span>
                </div>
                <div className="flex items-center justify-between text-xs text-black/60">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-center text-black/35 mt-3 leading-normal">
                  Payments are secure. No charges will be placed until service completion is confirmed.
                </p>
              </div>
            </div>
