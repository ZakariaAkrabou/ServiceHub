...
function statusBadge(status: string) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] sm:text-xs font-semibold ring-1 ring-inset ${styles[status] ?? "bg-neutral-50 text-neutral-700 ring-neutral-200/50"}`}>
...
const ProviderBookings: React.FC = () => {
  ...
  const { data: response, isLoading } = useGetProviderBookingsQuery(undefined, {
    skip: !token,
  });
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      rows = rows.filter((b) => {
        ...
        return clientName.includes(q) || serviceName.includes(q) || b._id.toLowerCase().includes(q);
      });
    }
    return rows;
      ...
      <div className="flex flex-col gap-6 pb-10">
              ...
              {isLoading ? (
                <tr>
                  <td colSpan={5} className="px-5 py-10 text-center">
                    <div className="flex flex-col items-center justify-center gap-3 text-[#9a9a9a]">
                ...
                paginated.map((b) => {
                  ...
                  const bookingDate = b.booking_time
                    ? new Date(b.booking_time).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })
                    : "—";
                  const bookingTime = b.booking_time
                    ? new Date(b.booking_time).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })
                    : "";

                  return (
                    <tr key={b._id} className="group transition-colors hover:bg-[#faf9f7] bg-white">
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-[11px] text-[#9a9a9a] mt-0.5 font-mono truncate max-w-32">{b._id}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <div className="text-[#1a1a1a] text-sm">{bookingDate}</div>
                        <div className="text-[11px] text-[#5f5f5f] mt-0.5">{bookingTime}</div>
                        <div className="md:hidden text-[11px] text-[#c9a84c] mt-0.5 max-w-30 truncate">{serviceName}</div>
                      </td>
                      <td className="px-5 py-4">
                        ...
                        <div className="flex items-center justify-end gap-1 sm:gap-2">
                          {b.status === "pending" && (
                            <>
                              <button
                                ...
                                className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-emerald-50 text-emerald-600 transition-colors hover:bg-emerald-100 disabled:opacity-50"
                              >
                              ...
                              <button
                                ...
                                className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-red-50 text-red-600 transition-colors hover:bg-red-100 disabled:opacity-50"
                              >
                                <X className="h-4 w-4" />
                              </button>
                            </>
                          ...
                          {b.status === "confirmed" && (
                            <button
                              ...
                              className="inline-flex px-3 py-1.5 items-center justify-center rounded-full bg-blue-50 text-blue-600 text-xs font-medium transition-colors hover:bg-blue-100 disabled:opacity-50"
                            >
                              Complete
                            </button>
                          )}
        ...
        <div className="flex justify-end items-center gap-2 mt-4">
          ...
          <span className="text-[#5f5f5f] text-sm">
            Page <span style={{ color: gold, fontWeight: 600 }}>{page}</span> of {totalPages}
          </span>
          <button
            className="rounded-lg border border-[#e9e3d3] px-3 py-1.5 text-sm text-[#1a1a1a] bg-white disabled:opacity-50"
